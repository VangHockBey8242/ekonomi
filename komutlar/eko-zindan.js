...
`)
.setFooter("Airla", client.user.avatarURL())
.setTimestamp()
)
...
if(args[0] === "örümcek") {
  if(!elmaskılıç) return message.reply(`Hiç elmas kılıcınız bulunmamaktadır! s!market`)
...
if(args[0] === "creeper") {
      if(!demirkılıç) return message.reply(`Hiç taş baltanız bulunmamaktadır! s!market`)
...
`)
      ...
      if(args[0] === "enderman") {
    if(!demirkılıç) return message.reply(`Hiç demir kılıcınız bulunmamaktadır! s!market`)
